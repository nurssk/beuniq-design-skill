...
Hard gate: before any UI edit, redesign, polish, cleanup, or fix pass, the agent must ask the user for missing style choices. Do not edit files, run fixes, or proceed with defaults until the user answers, unless the user explicitly says "choose defaults" or "no questions".

1. If the task asks to change, polish, fix, redesign, or make the UI pass BeUniq, first check whether the target repo has `PRODUCT.md`, `DESIGN.md`, a non-TBD `Selected Style Profile`, a non-TBD `Company Style Reference`, a non-TBD `Design System Source`, concrete component styles, and a non-TBD `Motion Style`. If all exist, read them and use them as constraints.
...
3. If `PRODUCT.md`, `DESIGN.md`, `Selected Style Profile`, `Company Style Reference`, `Design System Source`, component style choices, or `Motion Style` are missing or `TBD`, the first visible response must ask only the missing pieces. At minimum, when `Company Style Reference` is missing or `TBD`, ask the Company Style Category Choice; when component styles are missing, ask the Component Style Choice and include shadcn/Radix options if detected; when `Motion Style` is missing, ask the Motion Style Choice and include Framer Motion options if detected. Stop after asking and wait for the user's answers unless the user explicitly says to choose defaults.
4. Read `${CLAUDE_SKILL_DIR}/references/style-profiles.md` before presenting base profile options or explaining a profile. Read `${CLAUDE_SKILL_DIR}/references/company-style-references.md` before presenting company-style categories or companies. After the user chooses a company category, present only the companies inside that category, plus `nothing from this list`; do not present all companies at once. Read `${CLAUDE_SKILL_DIR}/references/component-patterns.md` before asking about component/block or motion examples. The choice must include any missing BeUniq base/existing/custom profile choice, company-style category then company or `nothing from this list`, concrete component choices for relevant components and sections, plus concrete motion choices when a motion stack is installed.
...
7. Create `PRODUCT.md` and `DESIGN.md` in the target repo before making UI edits. Use `${CLAUDE_SKILL_DIR}/scripts/beuniq-init.ts` when practical, then fill missing sections from the user's answers and the selected component/motion style profile. Do not overwrite existing context files unless the user explicitly asks.
...
13. Re-run the checker after each fix pass. Continue until the score passes or the remaining findings are visual-only/human-judgment.
...
If context files already exist, read them instead of asking repeated intake questions. If `Company Style Reference` is absent or `TBD`, still ask the two-step company-style choice even when `Selected Style Profile` is already present. If `Design System Source`, button style, field style, card style, modal style, or `Components` is absent or `TBD`, ask component style choices; include shadcn/Radix options when detected, otherwise ask generic component-source choices. If `Motion Style` is absent or `TBD`, ask motion style choices; include Framer Motion options when detected, otherwise include `motion-none` and conservative functional choices. If only one file exists, ask only for the missing information and create only the missing file.
...
Ask these whenever the matching answer is not already recorded in `DESIGN.md`; tailor the options using the lightweight scan.

1. **Shadcn/Radix component style:** if `components/ui`, `components.json`, `@radix-ui/*`, `class-variance-authority`, or `tailwind-merge` is detected, ask whether to derive style from existing shadcn variants, BeUniq base patterns mapped onto shadcn, custom overrides, or `nothing from this list`. If shadcn/Radix is not detected, ask the same Component Style Choice with existing components, BeUniq base patterns, custom overrides, or `nothing from this list`.
2. **Framer Motion style:** ask which animation approach to use: `motion-none`, `motion-functional-microinteractions`, `motion-native-sheets`, `motion-product-reveal`, `motion-scroll-subtle`, or `nothing from this list`. Mention Framer Motion specifically when `framer-motion` or `motion` is detected.
