---
name: beuniq-design
description: Rule-based frontend design, taste, motion-craft, landing-copy audit, concrete component/block pattern selection, two-step company-style reference choice, and project-context workflow with targeted cleanup for detecting generic AI-generated UI patterns without AI APIs. Use when Codex is asked to make a React, Next.js, Vite, HTML, CSS, SCSS, or Tailwind UI feel less AI-generated, choose concrete UI examples for buttons, fields, cards, modals, header, hero, pricing, how-it-works, text animation, scroll behavior, or company-inspired style references, improve taste, review motion craft, run an AI-slop/design-quality check, initialize PRODUCT.md/DESIGN.md design context, enforce BeUniq design criteria, or iterate until AI slop is at or below 20. For UI change, polish, redesign, cleanup, or fix passes, first read existing PRODUCT.md/DESIGN.md and Selected Style Profile; if missing, ask BeUniq Design Intake/style-profile/company-style-category choice and wait before editing or running fixes.
---
...
3. Read `references/style-profiles.md` before presenting base profile options or explaining a profile. Read `references/company-style-references.md` before presenting company-style categories or companies. After the user chooses a company category, present only the companies inside that category, plus `nothing from this list`; do not present all companies at once. Read `references/component-patterns.md` before asking about component/block examples. The choice must include: BeUniq base profile, existing component collection, or custom/reference; company-style category then company or `nothing from this list`; plus concrete pattern choices for relevant components and sections.
...
Ask this after Style Profile Choice and before concrete component patterns whenever `Selected Style Profile` is missing or the user asks to choose a visual reference. Use `references/company-style-references.md`. This is a two-step choice: category first, company second.
